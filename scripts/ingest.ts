...
import {
  bestPairForMint,
  socialsFromPair,
} from "../src/lib/sources/dexscreener";
import {
  dailyOHLCV,
  tokenInfo,
  poolsForToken,
  poolListings,
} from "../src/lib/sources/geckoterminal";
import {
  DATA_RPC_URL,
...
import {
  ...
  searchNews,
  filterForProject,
} from "../src/lib/sources/newswire";
import { tokenReport } from "../src/lib/sources/rugcheck";
import {
  jupTokenSearch,
  jupPrices,
  jupVolume,
} from "../src/lib/sources/jupiter";
...
/**
 * Community listings (`source = "submitted"`) are real tokens that did not
 * launch through MetaDAO. The chain and market steps treat them like any
 * other project; the steps that consult MetaDAO's own records must not,
 * because those records are keyed by symbol or slug and a submitted token
 * could share either with a launch it has nothing to do with.
 */
const isSubmitted = (p: { source: string | null }) => p.source === "submitted";
...
async function main() {
  ...
  for (const p of discovered) {
    ...
    if (
      p.treasury_value_usd != null &&
      recordTreasurySnapshot(id, now(), p.treasury_value_usd)
    ) {
      treasuryMoves++;
    }
  }
  ...
  for (const p of allProjects()) {
    ...
    if (usdc == null) {
      console.log(`      ${p.name}: treasury vault unreadable`);
      continue;
    }
    recordTreasurySnapshot(p.id, now(), usdc);
    onchainTreasuries++;
    console.log(
      `      ${p.name}: treasury $${usdc.toLocaleString()} (on-chain)`,
    );
  }
  if (onchainTreasuries)
    console.log(`      ${onchainTreasuries} treasury balance(s) read on-chain`);
  ...
  for (const p of projects) {
    if (!p.mint || isSubmitted(p)) continue;
    ...
    if (!s) {
      console.log(`      ${p.name}: supply unavailable`);
      continue;
    }
    ...
    const lockedPct =
      s.teamPackage && s.totalSupply
        ? (s.teamPackage / s.totalSupply) * 100
        : 0;
    console.log(
  ...
  for (const p of allProjects()) {
    ...
    if (!r) {
      console.log(
        `      ${p.name} (${p.symbol ?? "?"}): no raise record — add one to RAISES`,
      );
      continue;
    }
    const closedTs = r.closed
      ? Math.floor(Date.parse(`${r.closed}T00:00:00Z`) / 1000)
      : undefined;
    upsertProject({
      slug: p.slug,
      name: p.name,
    ...
    raiseCount++;
    if (r.noRaise) {
      console.log(`      ${p.name}: no raise (${r.note?.slice(0, 40)}…)`);
      continue;
    }
  ...
  for (const p of allProjects()) {
    ...
    if (pair) {
      ...
      console.log(
        `      ${p.name}: $${pair.priceUsd} mcap=${mcap ? Math.round(mcap).toLocaleString() : "?"} (${src})`,
      );
    } else {
      // tokens that trade only on MetaDAO's own AMM: fall back to Jupiter
      const [prices, toks] = [
        await jupPrices([p.mint]),
        await jupTokenSearch(p.mint),
      ];
      ...
      if (price != null || tok) {
        ...
        `,
          ...
          fdv,
          tok?.liquidity ?? null,
          jupVolume(tok?.stats24h),
          tok?.stats1h?.priceChange ?? null,
  ...
  if (!FAST) {
    ...
    for (const p of allProjects()) {
      ...
      const ranked = [
        ...(p.pool_address
          ? [{ address: p.pool_address, createdAt: 0, liquidity: Infinity }]
          : []),
        ...pools.filter(
          (x) => x.address !== p.pool_address && x.liquidity >= 1,
        ),
      ].slice(0, 5);
      ...
      const ins = d.prepare(
        "INSERT OR REPLACE INTO candles (project_id, ts, o,h,l,c,v) VALUES (?,?,?,?,?,?,?)",
      );
    ...
    if (history.size === 0) {
      console.log(
        "      market-data unavailable — keeping GeckoTerminal range only",
      );
    } else {
      const ins = d.prepare(
      ...
      if (!largest.length) {
        ...
        console.log(
          `      ${p.name}: ${holderCount ?? "?"} holders` +
            (gt?.top10Pct != null
              ? `, top10 ${gt.top10Pct.toFixed(1)}% (geckoterminal)`
              : "") +
  ...
  if (!FAST) {
    ...
    for (const p of allProjects()) {
      ...
      if (failed) {
        console.log(`      ${p.name}: lookup failed, keeping stored venues`);
        continue;
      }
      ...
      console.log(
        `      ${p.name}: ${rows.length} venues (${cex} CEX, ${cg.length} from CoinGecko)`,
      );
    }
  }
  ...
  if (!FAST) {
    ...
    for (const p of allProjects()) {
      ...
      `,
        ...
        r.freezeAuthorityEnabled == null
          ? null
          : r.freezeAuthorityEnabled
            ? 1
            : 0,
        r.lpLockedPct,
        r.totalHolders,
      ...
      if (r.holders.length) {
        ...
        r.holders.forEach((h, i) => {
          ...
          const label =
            h.label ??
            KNOWN_WALLETS[h.owner ?? ""]?.label ??
            (h.insider ? "Insider network" : null);
          insH.run(
            p.id,
            i + 1,
            h.address,
  ...
  // 4d. governance
  console.log("[4d] governance…");

  for (const p of allProjects()) {
    if (!p.mint) continue;

    try {
      const governance = await detectGovernance(p);

      saveGovernance(p.id, governance);

      console.log(
        `      ${p.name}: ${governance.type}` +
          `${governance.protocol ? ` · ${governance.protocol}` : ""}` +
          ` · ${governance.proposal_count} proposals`,
      );
    } catch (error) {
      console.error(`      ${p.name}: governance scan failed`, error);
    }
  }
  ...
  for (const p of allProjects()) {
    if (p.github) {
      ...
      if (owner) {
        ...
        if (gs) {
          ...
          d.prepare(
            "DELETE FROM events WHERE project_id = ? AND type = 'github_commit'",
          ).run(p.id);
    ...
    if (p.website && !FAST) {
      ...
      if (feed) {
        ...
        for (const it of items)
          insNews.run(p.id, it.ts, "news", it.title, null, it.url);
        if (items.length)
          console.log(`      ${p.name}: ${items.length} feed items`);
      }
    }
    ...
    if (p.raise_end_ts) {
      recordStructuralEvent(
        ...
        "Raise closed",
        p.raise_amount_usd
          ? `$${Math.round(p.raise_amount_usd).toLocaleString()} raised`
          : null,
      );
    }
    if (p.launch_ts) {
  ...
  if (!FAST) {
    ...
    for (const p of allProjects()) {
      ...
      if (all.length) {
        matched++;
        console.log(
          `      ${p.name}: ${all.length} article${all.length === 1 ? "" : "s"} (${added} new)`,
        );
      }
    }
    console.log(
      `      ${matched} projects with coverage, ${rows} rows written`,
    );
  }
...
function generateObservations() {
  ...
  const ts = now();
  const ins = d.prepare(
    "INSERT INTO observations (project_id, ts, kind, text) VALUES (?,?,?,?)",
  );
  ...
  for (const p of allProjects()) {
    ...
    const candles = d
      .prepare("SELECT ts,c,h,v FROM candles WHERE project_id = ? ORDER BY ts")
      .all(p.id) as { ts: number; c: number; h: number; v: number }[];
    const holders = d
      .prepare(
        "SELECT ts, holder_count, top10_pct FROM holder_snapshots WHERE project_id = ? ORDER BY ts",
      )
      .all(p.id) as {
      ts: number;
    ...
    if (candles.length >= 8) {
      ...
      if (pv7 > 0 && v7 / pv7 > 1.5) {
        ins.run(
          ...
          ts,
          "momentum",
          `Weekly volume up ${Math.round((v7 / pv7 - 1) * 100)}% vs the prior week.`,
        );
      }
      const ath = Math.max(...candles.map((c) => c.h));
      const last = candles[candles.length - 1].c;
      if (last >= ath * 0.95)
        ins.run(p.id, ts, "price", "Trading within 5% of all-time high.");
      ...
      if (d30.length >= 30) {
        ...
        if (Math.abs(chg) > 0.4) {
          ins.run(
            ...
            ts,
            "price",
            `Price ${chg > 0 ? "up" : "down"} ${Math.abs(Math.round(chg * 100))}% over the last 30 days.`,
          );
        }
      }
    }
    if (holders.length >= 2) {
      ...
      if (a.holder_count && b.holder_count && a.ts < b.ts - 86400) {
        ...
        if (Math.abs(chg) > 0.05) {
          ins.run(
            ...
            ts,
            "holders",
            `Holder count ${chg > 0 ? "grew" : "declined"} ${Math.abs(Math.round(chg * 100))}% since tracking began.`,
          );
        }
      }
      ...
      if (t10 != null && t10 > 60)
        ins.run(
          ...
          ts,
          "risk",
          `High concentration: top 10 accounts hold ${t10.toFixed(0)}% of supply.`,
        );
    }
    const snap = snaps[0];
    if (
      ...
      snap.vol24h != null &&
      snap.liquidity_usd > 0 &&
      snap.vol24h / snap.liquidity_usd > 2
    ) {
      ins.run(
        p.id,
        ts,
        "momentum",
        "24h volume exceeds 2× pool liquidity — unusually active trading.",
      );
    }
    const gh = d
      .prepare(
        "SELECT last_push_ts FROM github_snapshots WHERE project_id = ? ORDER BY ts DESC LIMIT 1",
      )
      .get(p.id) as { last_push_ts: number | null } | undefined;
    if (gh?.last_push_ts && ts - gh.last_push_ts < 3 * 86400) {
      ins.run(
        p.id,
        ts,
        "github",
        "Active development: code pushed within the last 3 days.",
      );
    }
  }
}
  ...
  for (const p of allProjects()) {
    ...
    for (const col of prof.dead) {
      ...
      if (had != null && had !== "") {
        cleared++;
        console.log(
          `      ${p.name}: cleared dead ${col} (${String(had).slice(0, 48)})`,
        );
      }
    }
  }
