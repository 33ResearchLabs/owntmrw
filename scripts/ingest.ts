...
import { db, allProjects, saveGovernance } from "../src/lib/db";
import { detectGovernance } from "../src/lib/governance/detector";
...
async function main() {
  ...
  console.log("======================================");
  console.log("GOVERNANCE ONLY INGEST");
  console.log("======================================");
  ...
  for (const project of projects) {
    if (!project.mint) {
      console.log(`SKIP ${project.name}: no mint`);
      continue;
    }

    try {
      console.log(`\nScanning ${project.symbol ?? project.name}...`);

      const governance = await detectGovernance(project);

      saveGovernance(project.id, governance);

      console.log(`  type: ${governance.type}`);
      console.log(`  protocol: ${governance.protocol ?? "unknown"}`);
      console.log(`  voting: ${governance.voting_model ?? "unknown"}`);
      console.log(`  proposals: ${governance.proposal_count}`);
